...
class MensualiteController {
    ...
    public function classDashboard($classeId) {
        ...
        // Batch fetch all payments for this class to avoid N+1
        $stmt = $db->prepare("
        ...
        ");
        $stmt->execute(['classe_id' => $classeId, 'annee_id' => $activeYear['id']]);
        $allPayments = $stmt->fetchAll(PDO::FETCH_GROUP | PDO::FETCH_ASSOC);

        foreach ($eleves as &$eleve) {
            $id = $eleve['id_etude'];
            $eleve['payments'] = [];
            if (isset($allPayments[$id])) {
                foreach ($allPayments[$id] as $p) {
                    $eleve['payments'][$p['mois_ou_sequence']] = ['total' => $p['total']];
                }
            }
        }
