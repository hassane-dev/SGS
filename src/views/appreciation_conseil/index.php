  ...
  <div class="pc-content">
    ...
    <?php if (!empty($_SESSION['success_message'])): ?>
      <div class="alert alert-success alert-dismissible fade show" role="alert">
        <i class="ph-duotone ph-check-circle me-2 fs-5 align-middle"></i><?= htmlspecialchars($_SESSION['success_message']) ?>
    ...
    <?php if (!empty($_SESSION['error_message'])): ?>
      <div class="alert alert-danger alert-dismissible fade show" role="alert">
        <i class="ph-duotone ph-warning-circle me-2 fs-5 align-middle"></i><?= htmlspecialchars($_SESSION['error_message']) ?>
        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
      </div>
    ...
    <?php if (!empty($error)): ?>
      <div class="alert alert-warning border-start border-4 border-warning mb-4" role="alert">
        <i class="ph-duotone ph-info me-2 fs-5 align-middle"></i><?= htmlspecialchars($error) ?>
      </div>
    <?php endif; ?>
      ...
      <div class="col-md-12">
        <div class="card">
          <div class="card-header pb-2 border-bottom-0">
            <h5><i class="ph-duotone ph-sliders me-2 text-primary"></i><?= _("Sélection de la Classe et de la Séquence") ?></h5>
          </div>
          <div class="card-body">
            <form method="GET" action="/appreciation-conseil" class="row g-3 align-items-end">
              <div class="col-md-5">
                <label for="classe_id" class="form-label fw-bold"><i class="ph-duotone ph-chalkboard-teacher me-1 text-primary"></i><?= _("Classe principale") ?></label>
              ...
              <div class="col-md-5">
                <label for="sequence_id" class="form-label fw-bold"><i class="ph-duotone ph-calendar me-1 text-info"></i><?= _("Séquence académique") ?></label>
              ...
              <div class="col-md-2 d-grid">
                <button type="submit" class="btn btn-outline-primary d-inline-flex align-items-center justify-content-center">
        ...
        <div class="col-md-12">
          <div class="card">
            <div class="card-header d-flex align-items-center justify-content-between">
              <h5>
                <i class="ph-duotone ph-user-list me-2 text-primary"></i>
                <?= htmlspecialchars(trim(($selected_classe['niveau'] ?? '') . ' ' . ($selected_classe['serie'] ?? '') . ' ' . ($selected_classe['numero'] ?? ''))) ?>
                &mdash; <span class="text-primary fw-bold"><?= htmlspecialchars($selected_sequence['nom'] ?? '') ?></span>
              </h5>
              <span class="badge bg-light-primary text-primary fs-6 fw-bold">
            ...
            <div class="card-body p-0">
              ...
              <?php else: ?>
                <form action="/appreciation-conseil/save" method="POST">
                    ...
                    <table class="table table-hover align-middle mb-0">
                      <thead class="table-light">
                        <tr>
                          ...
                          <th style="width: 30%;"><?= _("Élève") ?></th>
                          <th style="width: 50%;"><?= _("Appréciation du conseil de classe") ?></th>
                          <th style="width: 15%;" class="text-center"><?= _("Statut Bulletin") ?></th>
                        </tr>
                      </thead>
                      <tbody>
                          ...
                          <tr>
                            ...
                            <td>
                              <div class="d-flex align-items-center">
                                <?php if (!empty($e['photo'])): ?>
                                  <img src="<?= htmlspecialchars($e['photo']) ?>" alt="Photo" class="rounded-circle me-2" style="width: 40px; height: 40px; object-fit: cover;">
                                <?php else: ?>
                                  <div class="avatar-soft-primary rounded-circle me-2 d-flex align-items-center justify-content-center fw-bold" style="width: 40px; height: 40px; background-color: #eef2f6; color: #4680ff;">
                                    <?= strtoupper(substr($e['prenom'] ?? '', 0, 1) . substr($e['nom'] ?? '', 0, 1)) ?>
                                  </div>
                                <?php endif; ?>
                                <div>
                                  <div class="fw-bold text-dark"><?= htmlspecialchars(($e['nom'] ?? '') . ' ' . ($e['prenom'] ?? '')) ?></div>
                                  <small class="text-muted d-block">ID: #<?= $eId ?></small>
                                </div>
                              </div>
                            </td>
                            <td>
                              <?php if ($isLocked): ?>
                                <div class="p-2 rounded bg-light border text-muted">
                                  <i class="ph-duotone ph-lock me-1 text-warning"></i><?= htmlspecialchars($apprecVal ?: _('Aucune appréciation enregistrée.')) ?>
                                </div>
                            ...
                            <td class="text-center">
                              <?php if ($statutVal === 'publie'): ?>
                                <span class="badge bg-light-success text-success fs-6"><i class="ph-duotone ph-globe me-1"></i><?= _("Publié") ?></span>
                              <?php elseif ($statutVal === 'valide'): ?>
                                <span class="badge bg-light-info text-info fs-6"><i class="ph-duotone ph-check-circle me-1"></i><?= _("Validé") ?></span>
                              <?php else: ?>
                                <span class="badge bg-light-secondary text-secondary fs-6"><i class="ph-duotone ph-clock me-1"></i><?= _("Provisoire") ?></span>
                              <?php endif; ?>
                            </td>
                          </tr>
                        <?php endforeach; ?>
                  ...
                  <div class="card-footer d-flex align-items-center justify-content-between p-3 border-top">
                    <span class="text-muted small">
                      <i class="ph-duotone ph-info me-1 text-primary"></i><?= _("Les appréciations enregistrées s'inscriront directement sur le bulletin officiel de l'élève.") ?>
                    </span>
                    <button type="submit" class="btn btn-primary btn-lg d-inline-flex align-items-center">
                      <i class="ph-duotone ph-floppy-disk me-2 fs-4"></i><?= _("Enregistrer les appréciations") ?>
                    </button>
                  </div>
                </form>
              <?php endif; ?>
