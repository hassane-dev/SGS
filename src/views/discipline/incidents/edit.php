<?php
$title = _("Modifier l'Incident Disciplinaire");
                ...
                <div class="row align-items-center">
                    <div class="col-md-12">
                        <div class="page-header-title">
                            <h5 class="m-b-10"><?= _("Modification d'Incident #") ?><?= $incident['id'] ?></h5>
                        </div>
                        <ul class="breadcrumb">
                            ...
                            <li class="breadcrumb-item" aria-current="page"><?= _("Modifier #") ?><?= $incident['id'] ?></li>
                        </ul>
                    </div>
                </div>
            </div>
    ...
    <?= csrf_field() ?>
            ...
            <div class="row">
                        ...
                        <div class="card-body">
                            <div class="mb-3">
                                ...
                                <select name="type_incident_id" id="type_incident_id" class="form-select" required>
                                    ...
                                    <?php foreach ($typesIncidents as $ti): ?>
                                        <option value="<?= $ti['id'] ?>" <?= ($incident['type_incident_id'] == $ti['id']) ? 'selected' : '' ?>>
                            ...
                            <div class="row g-3 mb-3">
                                <div class="col-md-6">
                                    ...
                                    <input type="date" name="date_incident" id="date_incident" class="form-control" value="<?= htmlspecialchars($incident['date_incident']) ?>" required>
                                </div>
                                <div class="col-md-6">
                                    <label for="heure_incident" class="form-label"><?= _("Heure (optionnel)") ?></label>
                                    <input type="time" name="heure_incident" id="heure_incident" class="form-control" value="<?= htmlspecialchars($incident['heure_incident'] ?? '') ?>">
                                </div>
                            </div>

                            <div class="mb-3">
                                <label for="lieu" class="form-label"><?= _("Lieu de l'Incident") ?></label>
                                <input type="text" name="lieu" id="lieu" class="form-control" value="<?= htmlspecialchars($incident['lieu'] ?? '') ?>" placeholder="<?= _('Ex: Salle 102, Cour de récréation, Refectoire...') ?>">
                            </div>

                            <div class="mb-3">
                                <label for="description_faits" class="form-label required-field"><?= _("Description Détaillée des Faits") ?></label>
                                <textarea name="description_faits" id="description_faits" rows="5" class="form-control" required><?= htmlspecialchars($incident['description_faits']) ?></textarea>
                            </div>
                        </div>
                    </div>
                </div>
                            ...
                            <div class="table-responsive">
                                <table class="table table-bordered align-middle" id="tableElevesImpliques">
                                    ...
                                    <tbody id="tbodyEleves">
                                        <?php foreach ($incident['eleves'] as $idx => $e): ?>
                                            <tr id="rowEleve_<?= $e['eleve_id'] ?>">
                                                <td>
                                                    <input type="hidden" name="eleves[<?= $idx ?>][eleve_id]" value="<?= $e['eleve_id'] ?>">
                                                    <div class="fw-bold"><?= htmlspecialchars($e['nom'] . ' ' . $e['prenom']) ?></div>
                                                </td>
                                                <td>
                                                    <select name="eleves[<?= $idx ?>][role_implication]" class="form-select form-select-sm">
                                                        <option value="auteur_principal" <?= ($e['role_implication'] === 'auteur_principal') ? 'selected' : '' ?>><?= _("Auteur Principal") ?></option>
                                                        <option value="co_auteur" <?= ($e['role_implication'] === 'co_auteur') ? 'selected' : '' ?>><?= _("Co-auteur") ?></option>
                                                        <option value="complice" <?= ($e['role_implication'] === 'complice') ? 'selected' : '' ?>><?= _("Complice") ?></option>
                                                        <option value="victime" <?= ($e['role_implication'] === 'victime') ? 'selected' : '' ?>><?= _("Victime") ?></option>
                                                        <option value="temoin" <?= ($e['role_implication'] === 'temoin') ? 'selected' : '' ?>><?= _("Témoin") ?></option>
                                                    </select>
                                                </td>
                                                <td>
                                                    <input type="text" name="eleves[<?= $idx ?>][observation_individuelle]" class="form-control form-select-sm" value="<?= htmlspecialchars($e['observation_individuelle'] ?? '') ?>" placeholder="<?= _('Observation spécifique...') ?>">
                                                </td>
                                                <td class="text-center">
                                                    <button type="button" class="btn btn-sm btn-icon btn-light-danger" onclick="removeEleveRow('<?= $e['eleve_id'] ?>')">
                ...
                <div class="col-12 text-end mb-4">
                    <a href="/discipline/incidents/show?id=<?= $incident['id'] ?>" class="btn btn-link text-muted me-2"><?= _("Annuler") ?></a>
                    <button type="submit" class="btn btn-primary btn-lg d-inline-flex align-items-center gap-2">
                        <i class="ph-duotone ph-floppy-disk fs-4"></i><?= _("Mettre à jour l'incident") ?>
                    </button>
                </div>
...
<script>
let eleveIndex = <?= count($incident['eleves']) ?>;
let addedEleveIds = new Set([<?= implode(',', array_column($incident['eleves'], 'eleve_id')) ?>]);
...
function addSelectedEleve() {
    ...
    if (addedEleveIds.has(parseInt(eleveId))) {
    ...
    addedEleveIds.add(parseInt(eleveId));
    eleveIndex++;
...
function removeEleveRow(eleveId) {
    ...
    if (row) {
        row.remove();
        addedEleveIds.delete(parseInt(eleveId));
    }
