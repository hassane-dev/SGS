...
<div class="container mx-auto">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold"><?= _('Contract Type Management') ?></h2>
    ...
    <div class="bg-white shadow-md rounded">
        <table class="min-w-full table-auto">
            <thead class="bg-gray-200">
                <tr>
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"><?= _('Label') ?></th>
                ...
                <?php foreach ($contrats as $contrat): ?>
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <?= $contrat['lycee_id'] ? _('School Specific') : _('Global') ?>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <a href="/contrats/edit?id=<?= $contrat['id_contrat'] ?>" class="text-indigo-600 hover:text-indigo-900"><?= _('Edit') ?></a>
                            <form action="/contrats/destroy" method="POST" class="inline-block ml-4" onsubmit="return confirm('<?= _('Are you sure?') ?>');">
