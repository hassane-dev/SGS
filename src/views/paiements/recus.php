            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <div class="row align-items-center">
                            <div class="col">
                                <h5><?= _('Liste des Reçus émis') ?></h5>
                            </div>
                            <div class="col-auto">
                                <form method="GET" action="/paiements/recus" class="d-flex">
                                    <input type="text" name="search" class="form-control me-2" placeholder="<?= _('N° Reçu ou Nom...') ?>" value="<?= htmlspecialchars($search) ?>">
                                    <button type="submit" class="btn btn-primary"><?= _('Chercher') ?></button>
                                </form>
                            </div>
                        </div>
                    </div>
                                        ...
                                        <?php foreach ($recus as $r): ?>
                                            <tr>
                                                ...
                                                <td class="text-center">
                                                    <a href="/recu/print?numero=<?= $r['recu_numero'] ?>" target="_blank" class="btn btn-icon btn-primary" title="<?= _('Imprimer le reçu') ?>">
                                                        <i class="ph-duotone ph-printer"></i>
                                                    </a>
                                                    <a href="/paiements/show/<?= $r['id_eleve'] ?>" class="btn btn-icon btn-light-secondary" title="<?= _('Historique élève') ?>">
                                                        <i class="ph-duotone ph-clock-counter-clockwise"></i>
                                                    </a>
                                                    <?php if (Auth::can('annuler_recu', 'paiement')): ?>
                                                        <button type="button" class="btn btn-icon btn-danger btn-cancel-recu" data-recu="<?= $r['recu_numero'] ?>" title="<?= _('Annuler le reçu') ?>">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title text-danger" id="cancelRecuModalLabel"><i class="ph-duotone ph-warning me-2"></i><?= _('Annulation de Reçu') ?></h5>
            ...
            <form id="cancelRecuForm" method="POST" action="/paiements/annuler-recu">
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label class="form-label fw-bold"><?= _('Numéro de reçu') ?></label>
                        <input type="text" name="recu_numero" id="modal_recu_numero" class="form-control" readonly required>
                    </div>
                    <div class="mb-3">
                        <label class="form-label fw-bold"><?= _("Motif de l'annulation") ?></label>
                        <textarea name="motif" class="form-control" rows="3" placeholder="<?= _("Saisir la raison de l'annulation...") ?>" required></textarea>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal"><?= _('Fermer') ?></button>
                    <button type="submit" class="btn btn-danger"><i class="ph-duotone ph-x-circle me-1"></i><?= _("Confirmer l'annulation") ?></button>
                </div>
            </form>
        </div>
    </div>
