<?php
// db/migrations/20240115_13_rename_and_extend_enseignant_matieres.php
...
function migrate_13($db) {
    ...
    if ($tableExists('enseignant_matieres') && !$tableExists('affectations_pedagogiques')) {
        ...
        echo "Migration 13: Renamed table enseignant_matieres to affectations_pedagogiques.\n";
    }
    ...
    if (!$tableExists('affectations_pedagogiques')) {
        ...
        $db->exec($sql);
        echo "Migration 13: Created table affectations_pedagogiques.\n";
    }
    ...
    $addColumnIfNeeded = function($table, $column, $definition) use ($db, $isSqlite) {
        ...
        if (!$columnExists) {
            ...
            echo "Migration 13: Added column $column to table $table.\n";
        }
    };

    // 2. Add columns if not present
    $addColumnIfNeeded('affectations_pedagogiques', 'volume_horaire_hebdo', 'DECIMAL(5,2) DEFAULT 0.00');
    $addColumnIfNeeded('affectations_pedagogiques', 'date_debut', 'DATE NOT NULL');
    ...
    $addColumnIfNeeded('affectations_pedagogiques', 'created_at', 'DATETIME DEFAULT CURRENT_TIMESTAMP');
    $addColumnIfNeeded('affectations_pedagogiques', 'updated_at', 'DATETIME DEFAULT CURRENT_TIMESTAMP');
    ...
    if (!$isSqlite) {
        try {
            ...
            if ($idx_stmt->fetch()) {
                ...
                echo "Migration 13: Dropped index unique_enseignant_matiere_annee.\n";
            }
        } catch (Exception $e) {
        ...
        try {
            ...
            if (!$idx_stmt->fetch()) {
                ...
                echo "Migration 13: Added index idx_aff_pedag_unique_active.\n";
            }
        } catch (Exception $e) {
            // Index exists
        }
        ...
        try {
            ...
            if (!$idx_stmt->fetch()) {
                ...
                echo "Migration 13: Added index idx_aff_pedag_enseignant.\n";
            }
        } catch (Exception $e) {
            // Index exists
        }
    ...
    echo "Migration 13: Table affectations_pedagogiques extension complete.\n";
}
